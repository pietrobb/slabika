...
This project attacks the input instead. Syllables and typographic break points
are computed from an explicit model of the language — vowel and diphthong
nuclei, syllabic `ŕ`, `ĺ`, `r`, `l`, consonant clusters and morpheme seams. The
typographic branch then applies the PSP rules independently; word forms are
generated by morphological expansion, not collected from existing hyphenated
sources. A generated list can be arbitrarily large and has, by
construction, zero internal inconsistency. Existing text is used only to decide
...
| `slabika.typo` | legal break points in the written word under PSP and typographic constraints |
