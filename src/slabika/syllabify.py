...
_VOZ_INFLECTIONS = frozenset({
    '', 'a', 'e', 'mi', 'ne', 'ni', 'och', 'om', 'ov', 'u', 'y',
...
_PRASTIT_INFLECTIONS = frozenset({
    'e', 'ene', 'ená', 'ené', 'eného', 'enej', 'enému', 'ení', 'enom', 'enou', 'enú',
    'ený', 'ených', 'eným', 'enými', 'i', 'iac', 'ia', 'il', 'ila', 'ili', 'ilo',
    'ime', 'ite', 'iť', 'í', 'ím', 'íme', 'íš', 'íte',
})
...
_ALZBETINSK_ENDINGS = _DOBR_INFLECTIONS
_MACARTHUR_OVSK_ENDINGS = _DOBR_INFLECTIONS
_LOS_ANGELES_SK_ENDINGS = frozenset({
    'ká', 'ké', 'kého', 'kej', 'kému', 'kí', 'kom', 'kou', 'kú',
    'ký', 'kých', 'kým', 'kými',
})
...
def _lexical_syllables(word: str) -> list[str] | None:
    ...
    if (
        folded.startswith('macarthurovsk')
        and folded[13:] in _MACARTHUR_OVSK_ENDINGS
    ):
        return [
            word[:3], word[3:5], word[5:8], word[8:11],
            *_syllabify_simple(word[11:]),
        ]
    if (
        folded.startswith('losangeles')
        and folded[10:] in _LOS_ANGELES_SK_ENDINGS
    ):
        return [
            word[:3], word[3:5], word[5:7], word[7:10],
            *_syllabify_simple(word[10:]),
        ]
...
def _alzbeta_parts(word: str) -> list[str] | None:
    ...
    if not folded.startswith('alžbet'):
        return None
    if (
        folded.startswith('alžbetínsk')
        and folded[10:] in _ALZBETINSK_ENDINGS
    ):
        return [
            word[:3], word[3:5], word[5:8],
            *_syllabify_simple(word[8:]),
        ]
...
# licensed environments: obo- before the p-roots below and in obo|zretný,
# otherwise mainly the pronoun stems mn- and vš-: bezo mňa, podo
...
_SK_K_SUFFIX_ENDINGS = frozenset({
    ...
    'ovo', 'ovosť', 'ovosti', 'ovostí',                         # vonkajš·kovo/-kovosť
...
_VALID_ONSETS = frozenset({
    'bd', 'bl', 'br', 'bz', 'ch', 'dr', 'fl', 'fr', 'gl', 'gr', 'kl', 'kr',
...
def _licenses_compositum(comp: str, rem: str) -> bool:
    ...
    if comp == 'kvart' and not reml.startswith('sext'):
        return False
    if comp == 'fajn' and reml.startswith('ov'):
        return False
    guarded_compounds = {
        'niekoľko': ('dň',),
        'ducha': ('prítom',),
...
def _strip_prefix(w: str) -> tuple[str, str] | tuple[None, None]:
    ...
    if wl.startswith('predobr') and wl[7:] in _DOBR_INFLECTIONS:
        return w[:3], w[3:]
    if wl.startswith('predn') and wl[5:] in _DOBR_INFLECTIONS:
        return None, None
    if any(
    ...
    for length, group in _PREFIXES_BY_LEN:
        ...
        if pfx in group:
            ...
            if pfx == 'po' and wl.startswith(('posteľ', 'postel', 'postieľ')):
                continue
            # Prakt-, prask- and prašn- are lexical stems. Prach and prak keep
            # their closed noun paradigms, prahnúť has contracted past forms,
            # and praštiť keeps its inflectional paradigm; none contains the
            # Slovak ancestor prefix pra-.
            if pfx == 'pra' and (
                reml.startswith(('hn', 'kt', 'sk', 'šn'))
            ...
            if pfx == 'do' and (reml == 'šky' or reml.startswith('štič')):
                continue
            # These pro-initial families have lexical stems, not the prefix pro-.
            ...
            if pfx == 'od' and (
                ...
                continue
            # Výšk- is the lexical family of výška, not the prefix vý- plus šk-.
            if pfx == 'vý' and wl.startswith('výšk'):
                continue
...
def _strip_suffix(w: str) -> tuple[str, str] | tuple[None, None]:
    ...
    for rhythmic_stem in _RHYTHMIC_SHORT_NY_STEMS:
        ...
        for tail_length in range(_MAX_INFLECTION + 1):
            ...
            if matched in _RHYTHMIC_SHORT_NIK:
                ...
                if last_nucleus not in _RHYTHMIC_LONG_NUCLEI:
                    continue
            # -ectvo keeps the agent-noun stem in -ec: svedec·tvo, kupec·tvo.
            if matched == 'ctv' and steml.endswith('e'):
                continue
...
def _split_potreb_family(word: str) -> tuple[list[str], str] | None:
    """Preserve po-|treb- through its o-, s-, u-, and ne- derivatives."""
    folded = word.casefold()
    parts = []
    offset = 0
    if folded.startswith('ne') and folded[2:].startswith(
        ('potreb', 'opotreb', 'spotreb', 'upotreb')
    ):
        parts.append(word[:2])
        offset = 2

    remainder = folded[offset:]
...
def get_morpheme_parts(word: str) -> list[str]:
    ...
    if potreb_family is not None:
        ...
        return [*prefixes, *get_morpheme_parts(remainder)]
    rastly_ending = next((
        ending for ending in _RASTLY_ENDINGS
        if wl.endswith(ending) and wl[:-len(ending)].endswith(('rast', 'rást'))
    ), None)
...
def phoneme_layout(word: str) -> tuple[list[str], list[int], list[int]]:
    ...
    for phoneme in phonemes:
        offsets.append(pos)
        pos += len(phoneme)
    return phonemes, offsets, _word_nuclei(word, phonemes)
    ...
    while index < len(phonemes):
        ...
        if (
            ...
            and not (
                pair == 'eu'
                and any(
                    wl.startswith(stem) and offset == hiatus_offset
                    for stem, hiatus_offset in _LEXICAL_FALLING_HIATUS
                )
            )
        ):
            out.append(pair)
...
def _syllabify_simple(word: str) -> list[str]:
    ...
    phonemes = _phonemes(word)
    n = len(phonemes)
    nuclei = _word_nuclei(word, phonemes)
