...
def test_slovak_hyphenation_golden_cases():
    expected = {
        ...
        "akciami": "ak·ci·a·mi",
        "funkciami": "funk·ci·a·mi",
        "licenciami": "li·cen·ci·a·mi",
        "klientka": "klient·ka",
...
def test_oneskor_family_keeps_the_documented_o_ne_skor_seams():
    expected = {
        "oneskorenej": "one·sko·re·nej",
        "oneskorenia": "one·sko·re·nia",
        "oneskorenom": "one·sko·re·nom",
        "oneskorenou": "one·sko·re·nou",
        "oneskorené": "one·sko·re·né",
        "oneskorený": "one·sko·re·ný",
        "oneskorených": "one·sko·re·ných",
        "oneskoril": "one·sko·ril",
        "oneskorila": "one·sko·ri·la",
        "oneskoriť": "one·sko·riť",
        "oneskorenie": "one·sko·re·nie",
        "oneskorenosť": "one·sko·re·nosť",
        "oneskorievať": "one·sko·rie·vať",
        "oneskorovať": "one·sko·ro·vať",
    }
    assert {word: hyphenate(word) for word in expected} == expected
    assert hyphenate("onehdy") == "oneh·dy"
    assert hyphenate("onen") == "onen"
    assert break_points("oneskorenie", contextual=True) == [1, 3, 6, 8]
...
def test_generated_inventory_divides_compounds_nobody_typed_in():
    ...
    expected = {
        ...
        "samostatnejšie": "sa·mo·stat·nej·šie",
        "najsamostatnejšia": "naj·sa·mo·stat·nej·šia",
        "svätostánku": "svä·to·stán·ku",
        "kyslotvárnych": "kys·lo·tvár·nych",
        "prdovzdorný": "pr·do·vzdor·ný",
        "hviezdoznalí": "hviez·do·zna·lí",
        "písmaznalí": "pís·ma·zna·lí",
        "svetaznalý": "sve·ta·zna·lý",
        "všeznalosti": "vše·zna·los·ti",
    }
    assert {word: hyphenate(word) for word in expected} == expected
    ...
    assert hyphenate("mimoriadnych") == "mi·mo·riad·nych"
    assert hyphenate("dotvára") == "do·tvá·ra"
...
def test_reviewed_derivatives_keep_their_established_stem_boundary():
    expected = {
        ...
        "starostlivosťou": "sta·rost·li·vos·ťou",
        "najstarostlivejších": "naj·sta·rost·li·vej·ších",
        "vrchnostenskú": "vrch·nos·ten·skú",
        "zosvetšteného": "zo·svet·šte·né·ho",
...
def test_next_reviewed_batch_keeps_clear_stems_and_compound_seams():
    expected = {
        "biliardtina": "bi·liard·ti·na",
        "biliardtinu": "bi·liard·ti·nu",
        "bohvieakej": "boh·vie·akej",
        "bohvieaký": "boh·vie·aký",
        "Kvetoslavov": "Kve·to·sla·vov",
        "dvojizbový": "dvoj·iz·bo·vý",
        "lesklý": "lesk·lý",
        "mrštnosťou": "mršt·nos·ťou",
        "nazlostená": "na·zlos·te·ná",
        "Praveľkosvetového": "Pra·veľ·ko·sve·to·vé·ho",
        "samomluvu": "sa·mo·mlu·vu",
    }
    assert {word: hyphenate(word) for word in expected} == expected
    assert hyphenate("bohviečo") == "boh·vie·čo"
    assert hyphenate("dvojitý") == "dvo·ji·tý"


def test_reviewed_prefix_seams_and_one_letter_syllabic_r():
    expected = {
        "doopekaj": "do·ope·kaj",
        "nadrie": "na·drie",
        "najneobyčajnejšie": "naj·ne·oby·čaj·nej·šie",
        "najneoceniteľnejšie": "naj·ne·oce·ni·teľ·nej·šie",
        "najneopísateľnejšie": "naj·ne·opí·sa·teľ·nej·šie",
        "najneotesanejšieho": "naj·ne·ote·sa·nej·šie·ho",
        "neexistoval": "ne·exis·to·val",
        "neočakávané": "ne·oča·ká·va·né",
        "reprodukuje": "re·pro·du·ku·je",
        "sedemmiestny": "se·dem·miest·ny",
        "zušľachtenejšie": "zu·šľach·te·nej·šie",
        "zušľachtiť": "zu·šľach·tiť",
        "rdúsenie": "rdú·se·nie",
        "rdúsite": "rdú·si·te",
        "rčenie": "rče·nie",
        "úhlavný": "úhlav·ný",
        "úhlavné": "úhlav·né",
        "úhlavného": "úhlav·né·ho",
        "úhlavnému": "úhlav·né·mu",
        "úhlavní": "úhlav·ní",
        "úhlavnom": "úhlav·nom",
        "úhlavných": "úhlav·ných",
        "úhlavným": "úhlav·ným",
        "úhlavnými": "úhlav·ný·mi",
    }
    assert {word: hyphenate(word) for word in expected} == expected
    assert hyphenate("rdúsenie", contextual=True) == "r·dú·se·nie"
    assert hyphenate("rčenie", contextual=True) == "r·če·nie"
...
def test_operator_approved_scale_1000_native_forms():
    expected = {
        "polizba": "pol·iz·ba",
        "podrobujete": "pod·ro·bu·je·te",
        "podrobenými": "pod·ro·be·ný·mi",
        "podrobujúc": "pod·ro·bu·júc",
        "podrobiť": "pod·ro·biť",
        "rozorvanej": "ro·zo·rva·nej",
        "rozorvane": "ro·zo·rva·ne",
        "rozorval": "ro·zo·rval",
        "hmkal": "hm·kal",
        "vyrve": "vy·rve",
        "vyrvite": "vy·rvi·te",
...
def test_poslat_uses_its_po_seam_while_postit_remains_lexical():
    expected = {
        "pošla": "po·šla",
        "nepošle": "ne·po·šle",
        "nepošlem": "ne·po·šlem",
        "nepošlete": "ne·po·šle·te",
        "neposlintané": "ne·po·slin·ta·né",
        "neposlúcha": "ne·po·slú·cha",
        "neposlúchajú": "ne·po·slú·cha·jú",
        "neposlúchal": "ne·po·slú·chal",
        "neposlúchali": "ne·po·slú·cha·li",
        "neposlúchnem": "ne·po·slúch·nem",
        "neuposlúchnutie": "ne·upo·slúch·nu·tie",
        "nepostí": "ne·pos·tí",
        "nepostia": "ne·pos·tia",
        "nepostil": "ne·pos·til",
...
def test_twenty_fourth_discovered_family_batch_keeps_only_clear_compound_seams():
    expected = {
        ...
        "veľadôstojní": "ve·ľa·dô·stoj·ní",
        "najveľadôstojnejší": "naj·ve·ľa·dô·stoj·nej·ší",
...
def test_batch_18_fajn_family_distinguishes_compounds_from_ov_derivatives():
    expected = {
...
def test_los_angeles_adjective_family_uses_its_slovak_pronunciation():
    expected = {
        "losangeleskej": "los·an·ge·les·kej",
        "losangeleská": "los·an·ge·les·ká",
        "losangeleského": "los·an·ge·les·ké·ho",
        "losangeleských": "los·an·ge·les·kých",
    }
    assert {word: hyphenate(word) for word in expected} == expected


def test_macarthur_adjective_family_preserves_the_name_pronunciation():
    assert hyphenate("macarthurovskej") == "mac·ar·thu·rov·skej"
...
def test_operator_approved_domestic_morpheme_seams_are_preferred():
    expected = {
        ...
        "najposlednejšie": "naj·po·sled·nej·šie",
        "najposlednejšieho": "naj·po·sled·nej·šie·ho",
        "najposlednejšiemu": "naj·po·sled·nej·šie·mu",
        "najposlednejšiu": "naj·po·sled·nej·šiu",
        "najposlednejšom": "naj·po·sled·nej·šom",
        "najposlednejšou": "naj·po·sled·nej·šou",
        "najposlednejší": "naj·po·sled·nej·ší",
        "najposlednejších": "naj·po·sled·nej·ších",
        "najposlednejším": "naj·po·sled·nej·ším",
        "najúhlavnejších": "naj·ú·hlav·nej·ších",
        "najúhlavnejším": "naj·ú·hlav·nej·ším",
        "naposledy": "na·po·sle·dy",
        "neposlednom": "ne·po·sled·nom",
        "obojpohlavná": "oboj·po·hlav·ná",
        "obojpohlavné": "oboj·po·hlav·né",
        "poslednýkrát": "po·sled·ný·krát",
        "zákryty": "zá·kry·ty",
    }
    assert {word: hyphenate(word) for word in expected} == expected
    assert 6 not in break_points("najposlednejšia", all_points=True)
    assert {2, 3} <= set(break_points("poslednýkrát", all_points=True))
    assert hyphenate("páčidlá") == "pá·čid·lá"
    assert {4, 5} <= set(break_points("páčidlá", all_points=True))


def test_rudo_compound_is_limited_to_the_hlav_family():
    assert hyphenate("Rudohlavcom") == "Ru·do·hlav·com"
    assert hyphenate("rudonosý") == "ru·do·no·sý"
...
def test_operator_approved_third_scale_1000_native_forms():
    expected = {
        ...
        "opotrebovaný": "o·po·tre·bo·va·ný",
        "opotrebovaných": "o·po·tre·bo·va·ných",
        "opotrebovaným": "o·po·tre·bo·va·ným",
        "opotrebovanými": "o·po·tre·bo·va·ný·mi",
        "opotrebovávané": "o·po·tre·bo·vá·va·né",
        "opotrebuje": "o·po·tre·bu·je",
        "opotrebúva": "o·po·tre·bú·va",
        "opotrebúvali": "o·po·tre·bú·va·li",
        "púštneho": "púšt·ne·ho",
        "púštny": "púšt·ny",
        "púštnych": "púšt·nych",
        "púštnym": "púšt·nym",
...
def test_operator_approved_foreign_divisions_are_pronunciation_guarded():
    expected = {
        "Adrienne": "Ad·rienne",
        "Blackburna": "Black·bur·na",
        "Blake": "Blake",
        "Cypress": "Cyp·ress",
        "Department": "De·part·ment",
        "Elenore": "Ele·nore",
        "Fahrenheita": "Fah·ren·hei·ta",
        "Fahrenheitovho": "Fah·ren·hei·tov·ho",
        "Fairfaxe": "Fair·fa·xe",
        "Fairfaxovci": "Fair·fa·xov·ci",
        "Fallbrook": "Fall·brook",
        "Fallbrookovej": "Fall·broo·ko·vej",
        "Fallbrooková": "Fall·broo·ko·vá",
        "Fallbrooku": "Fall·broo·ku",
        "Falls": "Falls",
        "Gleisdorf": "Gleis·dorf",
        ...
        "Joeovi": "Joe·o·vi",
        "Johnnie": "John·nie",
        "Joinvilla": "Join·vil·la",
        "Joinville": "Join·ville",
        "Joinvillovcov": "Join·vil·lov·cov",
        "Jonesa": "Jone·sa",
        "Jonesovi": "Jone·so·vi",
        "Jonesovia": "Jone·so·via",
        "Jowette": "Jo·wette",
        "Joyce": "Joyce",
        "Joycea": "Joy·cea",
        "Lockridge": "Lock·ridge",
        "Loire": "Loire",
        "Loira": "Loi·ra",
        "Lois": "Lois",
        "Loua": "Loua",
        "Louis": "Louis",
        "Louisa": "Lou·i·sa",
        "Louisovi": "Lou·i·so·vi",
        "Lounge": "Lounge",
        "Lowry": "Low·ry",
        "Loyoly": "Lo·yo·ly",
        "Loyseleura": "Loy·se·leu·ra",
        "Loyseleurovi": "Loy·se·leu·ro·vi",
        "Loyseleurovo": "Loy·se·leu·ro·vo",
        "MAINE": "MAINE",
        "Macquarie": "Mac·qua·rie",
        "Maelströme": "Mael·strö·me",
        "Maelströmu": "Mael·strö·mu",
        "Magoon": "Ma·goon",
        "Magoona": "Ma·goo·na",
        "Magoonom": "Ma·goo·nom",
        "Magoonovi": "Ma·goo·no·vi",
        "Mahalaleela": "Ma·ha·la·le·e·la",
        "Mahalaleelom": "Ma·ha·la·le·e·lom",
        "Mahalaleelov": "Ma·ha·la·le·e·lov",
        "Mahalaleelovi": "Ma·ha·la·le·e·lo·vi",
        "Mahalaleelovmu": "Ma·ha·la·le·e·lov·mu",
        "Main": "Main",
        "Maioranos": "Ma·io·ra·nos",
        "Maioranosa": "Ma·io·ra·no·sa",
        "Maisie": "Mai·sie",
        "Marlene": "Mar·lene",
        ...
        "against": "against",
        "albastone": "al·ba·stone",
        "aliquandiu": "ali·quan·diu",
        "aliquid": "ali·quid",
        "ambagesque": "am·ba·ges·que",
        "applausit": "ap·plau·sit",
        "excellence": "ex·cel·lence",
