...
def test_thirty_fifth_discovered_family_batch_keeps_only_clear_stan_and_spev_seams():
    expected = {
        ...
        "novoustanovený": "no·vo·u·sta·no·ve·ný",
...
def test_batch_154_keeps_prospiet_stem_and_proti_alkohol_seam():
    ...
    assert {word: hyphenate(word) for word in expected} == expected
    assert hyphenate("protiopatrenia") == "pro·ti·o·pat·re·nia"
...
def test_batch_156_keeps_psohlav_seam_and_foreign_consonant_graphemes():
    ...
    assert {word: hyphenate(word) for word in expected} == expected
    assert {word: get_syllables(word) for word in expected} == {
        word: result.casefold().split("·") for word, result in expected.items()
    }
...
def test_potreb_family_keeps_psp_attested_morpheme_boundary():
    expected = {
        ...
        "neopotrebovali": "ne·o·po·tre·bo·va·li",
        "neopotrebovateľný": "ne·o·po·tre·bo·va·teľ·ný",
        "neopotrebuje": "ne·o·po·tre·bu·je",
        "spotreba": "spo·tre·ba",
